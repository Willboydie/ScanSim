...
class ProjectionPath:
    def __init__(self):
        ...
        self.shape = defaults.PROJECTION_PATH_SHAPE  # "rectangle" or "circle"
        self.radius = defaults.PROJECTION_PATH_RADIUS  # used when shape is "circle"
        self.points = None

    def generate(self, plot=False):
        cx, cy = self.centre
        if self.shape == "circle":
            # Generate points uniformly around the circle
            # Angular step derived from linear increment: circumference = 2*pi*r
            circumference = 2 * np.pi * self.radius
            num_points = max(8, int(circumference / self.increment))
            theta = np.linspace(0, 2 * np.pi, num_points, endpoint=False)
            x = cx + self.radius * -np.cos(theta)
            y = cy + self.radius * np.sin(theta)
            self.points = np.column_stack((x, y))

            # Rotate circle clockwise by orientation (0–360deg) about centre
